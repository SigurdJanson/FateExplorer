...
/// <summary>
/// Represents one particular type of energy.
/// </summary>
public class EnergyViMo
{
    ...
    /// <summary>
    /// Unique identifier for the character attribute
    /// </summary>
    ...
    /// <summary>
    /// Regenerates energy points by drinking a potion.
    /// </summary>
    /// <param name="PotionQuality">Quality level of the potion.</param>
    /// <returns>The complete roll result; null if the energy represented by this object is not health.</returns>
    /// <exception cref="InvalidOperationException">Thrown if the energy type is Karma.</exception>"
    public RollResultViMo DrinkPotion(int PotionQuality)
    {
        if (Energy.Class == CharacterEnergyClass.KP) throw new InvalidOperationException("Karma cannot be regenerated drinking potions.");
        if (effectiveValue >= EffMax) return null;

        EnergyPotionRollM potionRoll = new(PotionQuality);
        potionRoll.Roll();
        RollResultViMo Result = new(potionRoll);
